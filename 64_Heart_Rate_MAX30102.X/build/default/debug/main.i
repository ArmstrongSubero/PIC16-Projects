...
{
    uint8_t val = 0;
    i2c_read_bytes(0x57, reg, &val, 1);
    return val;
}

static void writeRegister8(uint8_t reg, uint8_t value)
{
    i2c_write_bytes(0x57, reg, &value, 1);
}
...
{
    bitMask(0x09, 0xBF, 0x40);


    uint32_t start = millis();
    while ((millis() - start) < 100)
    {
        uint8_t resp = readRegister8(0x09);
        if ((resp & 0x40) == 0) break;
...
{
    writeRegister8(0x04, 0);
    writeRegister8(0x05, 0);
    writeRegister8(0x06, 0);
}

static _Bool MAX30102_Init(void)
{

    if (readRegister8(0xFF) != 0x15)
        return 0;
    ...
    bitMask(0x08, ((uint8_t)~0xE0), 0x40);
    bitMask(0x08, 0xEF, 0x10);




    bitMask(0x09, 0xF8, 0x03);
    ...
    bitMask(0x0A, 0x9F, 0x20);
    bitMask(0x0A, 0xE3, 0x04);
    bitMask(0x0A, 0xFC, 0x03);





    writeRegister8(0x0C, 0x24);
    writeRegister8(0x0D, 0x24);





    bitMask(0x11, 0xF8, 0x01);
    bitMask(0x11, 0x8F, (uint8_t)(0x02 << 4));
                               ...
                               uint8_t max_samples)
{
    ...
    uint8_t rd_ptr = readRegister8(0x06);
    uint8_t wr_ptr = readRegister8(0x04);
    ...
    {
        uint8_t buf[6];
        if (i2c_read_bytes(0x57, 0x07, buf, 6) != 0)
            break;
...
static _Bool checkForBeat(int32_t sample)
{
    _Bool beatDetected = 0;
...
static uint32_t beat_times[8];
static uint16_t beat_intervals[8];
...
static _Bool bpm_record_beat(uint32_t now_ms)
{
    if (beat_count > 0)
    {
        uint8_t prev = (beat_idx - 1) % 8;
        ...
        {
            uint32_t lo = (uint32_t)beat_avg_interval * 60 / 100;
            uint32_t hi = (uint32_t)beat_avg_interval * 140 / 100;
            if (interval < lo || interval > hi)
                return 0;
        }


        beat_intervals[beat_idx % 8] = (uint16_t)interval;
    }

    beat_times[beat_idx % 8] = now_ms;
    beat_idx++;
    if (beat_count < 8) beat_count++;


    if (beat_count >= 2)
    {
        uint32_t sum = 0;
        uint8_t valid = (beat_count < 8) ? beat_count - 1 : 8 - 1;
        for (uint8_t i = 0; i < valid; i++)
        {
            uint8_t idx = (beat_idx - 1 - i) % 8;
...
void main(void)
{
    ...
    while (1)
    {
        uint8_t n = MAX30102_check(red_buf, ir_buf, 8);
        if (n == 0)
        {
            _delay((unsigned long)((1)*(32000000/4000.0)));
            continue;
