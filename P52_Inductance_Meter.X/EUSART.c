...
void EUSART_Write_Unsigned_Integer(uint32_t value_to_send) {
    char buffer[20];
    sprintf(buffer, "%lu", value_to_send); // Format as unsigned long
    EUSART_Write_Text(buffer);             // Send the string
}


/*******************************************************************************
 * Function: void ftoa(char *buffer, float value, int precision)
 *
 * Converts a floating-point number to a string with the specified precision.
 * 
 * buffer: The destination buffer to store the resulting string.
...
/*******************************************************************************
 * Function: void EUSART_Write_Float(float value, int precision)
 *
 * Sends a floating-point number over UART with the specified precision.
 * 
 * value: The floating-point number to send.
 * precision: The number of decimal places to include.
 ******************************************************************************/
void EUSART_Write_Float(float value, int precision) {
    char buffer[32]; // Buffer to hold the resulting string
    ftoa(buffer, value, precision); // Convert float to string
    EUSART_Write_Text(buffer); // Send the string over UART
}
