...
static _Bool checkTimeoutExpired(VL53L0X_Dev_t *dev)
{
...
static _Bool getSpadInfo(VL53L0X_Dev_t *dev, uint8_t *count, _Bool *type_is_aperture);
...
static _Bool performSingleRefCal(VL53L0X_Dev_t *dev, uint8_t vhv_init_byte);
...
static _Bool setMeasurementTimingBudget(VL53L0X_Dev_t *dev, uint32_t budget_us);




static _Bool VL53L0X_Init(VL53L0X_Dev_t *dev)
{

    writeReg(dev, 0x89,
             readReg(dev, 0x89) | 0x01);
    ...
    writeReg(dev, 0x60,
             readReg(dev, 0x60) | 0x12);


    writeReg16(dev, 0x44, 32);

    writeReg(dev, 0x01, 0xFF);
    ...
    readMulti(dev, 0xB0, ref_spad_map, 6);

    writeReg(dev, 0xFF, 0x01);
    writeReg(dev, 0x4F, 0x00);
    writeReg(dev, 0x4E, 0x2C);
    writeReg(dev, 0xFF, 0x00);
    writeReg(dev, 0xB6, 0xB4);
    ...
    writeMulti(dev, 0xB0, ref_spad_map, 6);
    ...
    writeReg(dev, 0x84,
             readReg(dev, 0x84) & ~0x10);
    writeReg(dev, 0x0B, 0x01);

    dev->measurement_timing_budget_us = getMeasurementTimingBudget(dev);
    writeReg(dev, 0x01, 0xE8);
    setMeasurementTimingBudget(dev, dev->measurement_timing_budget_us);


    writeReg(dev, 0x01, 0x01);
    if (!performSingleRefCal(dev, 0x40)) return 0;

    writeReg(dev, 0x01, 0x02);
    if (!performSingleRefCal(dev, 0x00)) return 0;
...
static _Bool setMeasurementTimingBudget(VL53L0X_Dev_t *dev, uint32_t budget_us)
{
    ...
    if (en.final_range)
    {
        ...
        writeReg16(dev, 0x71, encodeTimeout(fr_timeout_mclks));
...
static _Bool setVcselPulsePeriod(VL53L0X_Dev_t *dev, uint8_t type, uint8_t period_pclks)
{
    ...
    if (type == 0)
    {
        switch (period_pclks)
        {
            case 12: writeReg(dev, 0x57, 0x18); break;
            case 14: writeReg(dev, 0x57, 0x30); break;
            case 16: writeReg(dev, 0x57, 0x40); break;
            case 18: writeReg(dev, 0x57, 0x50); break;
            default: return 0;
        }
        writeReg(dev, 0x56, 0x08);
        writeReg(dev, 0x50, vcsel_reg);

        uint16_t new_pr = timeoutUsToMclks(to.pre_range_us, period_pclks);
        writeReg16(dev, 0x51, encodeTimeout(new_pr));

        uint16_t new_msrc = timeoutUsToMclks(to.msrc_dss_tcc_us, period_pclks);
        writeReg(dev, 0x46, (new_msrc > 256) ? 255 : (new_msrc - 1));
    }
    else if (type == 1)
    {
        switch (period_pclks)
        {
            case 8:
                writeReg(dev, 0x48, 0x10);
                writeReg(dev, 0x47, 0x08);
                writeReg(dev, 0x32, 0x02);
                writeReg(dev, 0x30, 0x0C);
                writeReg(dev, 0xFF, 0x01);
                writeReg(dev, 0x30, 0x30);
                writeReg(dev, 0xFF, 0x00);
                break;
            case 10:
                writeReg(dev, 0x48, 0x28);
                ...
                break;
            case 12:
                writeReg(dev, 0x48, 0x38);
                writeReg(dev, 0x47, 0x08);
                writeReg(dev, 0x32, 0x03);
                writeReg(dev, 0x30, 0x08);
                writeReg(dev, 0xFF, 0x01);
                writeReg(dev, 0x30, 0x20);
                writeReg(dev, 0xFF, 0x00);
                break;
            case 14:
                writeReg(dev, 0x48, 0x48);
                writeReg(dev, 0x47, 0x08);
                writeReg(dev, 0x32, 0x03);
                writeReg(dev, 0x30, 0x07);
                writeReg(dev, 0xFF, 0x01);
                writeReg(dev, 0x30, 0x20);
                writeReg(dev, 0xFF, 0x00);
                break;
...
{
    if (type == 0)
        return (((readReg(dev, 0x50)) + 1) << 1);
    else if (type == 1)
        return (((readReg(dev, 0x70)) + 1) << 1);
    return 255;
}
...
static void startContinuous(VL53L0X_Dev_t *dev, uint32_t period_ms)
{
    ...
    if (period_ms != 0)
    {
        uint16_t osc_cal = readReg16(dev, 0xF8);
        if (osc_cal != 0) period_ms *= osc_cal;
        writeReg32(dev, 0x04, period_ms);
        writeReg(dev, 0x00, 0x04);
    }
    else
    {
        writeReg(dev, 0x00, 0x02);
    }
}
...
static uint16_t readRangeContinuousMm(VL53L0X_Dev_t *dev)
{
    ...
    uint16_t range = readReg16(dev, 0x14 + 10);
    writeReg(dev, 0x0B, 0x01);
    return range;
}
...
static _Bool timeoutOccurred(VL53L0X_Dev_t *dev)
{
    _Bool tmp = dev->did_timeout;
    dev->did_timeout = 0;
    return tmp;
}
...
static _Bool getSpadInfo(VL53L0X_Dev_t *dev, uint8_t *count, _Bool *type_is_aperture)
{
    ...
    {
        if (checkTimeoutExpired(dev)) return 0;
    }
...
static void getSequenceStepTimeouts(VL53L0X_Dev_t *dev, SeqEnables_t *en, SeqTimeouts_t *to)
{
    ...
    to->msrc_dss_tcc_mclks = readReg(dev, 0x46) + 1;
    to->msrc_dss_tcc_us = timeoutMclksToUs(to->msrc_dss_tcc_mclks, to->pre_range_vcsel_period_pclks);
    to->pre_range_mclks = decodeTimeout(readReg16(dev, 0x51));
    ...
    to->final_range_mclks = decodeTimeout(readReg16(dev, 0x71));
    if (en->pre_range)
...
static uint32_t timeoutMclksToUs(uint16_t mclks, uint8_t vcsel_pclks)
{
    uint32_t macro_ns = ((((uint32_t)2304 * (vcsel_pclks) * 1655) + 500) / 1000);
...
static _Bool performSingleRefCal(VL53L0X_Dev_t *dev, uint8_t vhv_init_byte)
{
    writeReg(dev, 0x00, 0x01 | vhv_init_byte);
