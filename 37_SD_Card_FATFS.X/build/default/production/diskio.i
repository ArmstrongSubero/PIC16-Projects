...
static __attribute__((inline)) uint8_t xfer_spi(uint8_t d) { return SPI_Exchange8bit(d); }
static void xmit_spi(uint8_t d) { (void)xfer_spi(d); }
static uint8_t rcv_spi(void) { return xfer_spi(0xFF); }

static void dly_100us(void) { _delay((unsigned long)((100)*(32000000/4000000.0))); }
# 29 "diskio.c"
    ...
    if (send_cmd((0x40+0), 0) == 1) {
        ...
        if (send_cmd((0x40+8), 0x1AA) == 1) {
            ...
            if (ocr[2] == 0x01 && ocr[3] == 0xAA) {
                for (tmr = 10000; tmr && send_cmd((0xC0+41), 1UL << 30); tmr--) dly_100us();

                if (tmr && send_cmd((0x40+58), 0) == 0) {
                    for (n = 0; n < 4; n++) ocr[n] = rcv_spi();
            ...
            uint8_t cmd;
            if (send_cmd((0xC0+41), 0) <= 1) { ty = 0x02; cmd = (0xC0+41); }
            else { ty = 0x01; cmd = (0x40+1); }

            for (tmr = 10000; tmr && send_cmd(cmd, 0); tmr--) dly_100us();

            if (!tmr || send_cmd((0x40+16), 512) != 0) {
                ty = 0;
            }
        }
    }
...
DRESULT disk_read(BYTE pdrv, BYTE* buff, LBA_t sector, UINT count)
{
    ...
    while (count--) {
        if (send_cmd((0x40+17), (uint32_t)sector) != 0) { deselect(); return RES_ERROR; }
...
DRESULT disk_write(BYTE pdrv, const BYTE* buff, LBA_t sector, UINT count)
{
    ...
    while (count--) {
        if (send_cmd((0x40+24), (uint32_t)sector) != 0) { deselect(); return RES_ERROR; }
