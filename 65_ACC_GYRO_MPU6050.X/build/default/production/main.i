...
static _Bool mpu_write_reg(uint8_t reg, uint8_t val)
{
    return (i2c_write_bytes(0x68, reg, &val, 1) == 0);
}

static _Bool mpu_read_reg(uint8_t reg, uint8_t *val)
{
    return (i2c_read_bytes(0x68, reg, val, 1) == 0);
}

static _Bool mpu_read_multi(uint8_t reg, uint8_t *buf, uint8_t len)
{
    return (i2c_read_bytes(0x68, reg, buf, len) == 0);
}
...
static _Bool mpu_frame_sane(const uint8_t *b, uint8_t len)
{
...
static _Bool mpu_read_raw(mpu_raw_t *o)
{
    uint8_t b[14];

    for (uint8_t attempt = 0; attempt < 3; attempt++)
    {
        if (mpu_read_multi(0x3B, b, 14) && mpu_frame_sane(b, 14))
        {
...
static _Bool mpu_wait_data_ready(uint16_t timeout_ms)
{
    uint8_t st = 0;
    ...
    {
        if (mpu_read_reg(0x3A, &st) && (st & 0x01))
            return 1;
        _delay((unsigned long)((1)*(32000000/4000.0)));
    }
    return 0;
...
static _Bool mpu_init(void)
{
    uint8_t who = 0;

    if (!mpu_read_reg(0x75, &who))
        return 0;
    ...
    if (!mpu_write_reg(0x6B, 0x01)) return 0;
    _delay((unsigned long)((5)*(32000000/4000.0)));


    if (!mpu_write_reg(0x1A, 0x03)) return 0;


    if (!mpu_write_reg(0x19, 9)) return 0;


    if (!mpu_write_reg(0x1B, 0x00)) return 0;


    if (!mpu_write_reg(0x1C, 0x00)) return 0;


    mpu_write_reg(0x6A, 0x00);
    mpu_write_reg(0x23, 0x00);
    mpu_write_reg(0x37, 0x00);
...
static _Bool mpu_calibrate(void)
{
    ...
    g_az_off_mg = saz / (int32_t)good - 1000;
