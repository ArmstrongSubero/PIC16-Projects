...
typedef enum {
    PWM_PRESCALE_1 = 0b00,
    PWM_PRESCALE_4 = 0b01,
    PWM_PRESCALE_16 = 0b10,
    PWM_PRESCALE_64 = 0b11
} PWM_Prescaler;
...
void PWM_Init(PWM_Channel channel, PWM_Timer timer, uint32_t freq_hz);


void PWM_SetDutyCycle(PWM_Channel channel, uint8_t duty_percent);


void PWM_SetDutyCycle10bit(PWM_Channel channel, uint16_t duty_value);
...
void PWM_SetOutputPin(PWM_Channel channel, volatile uint8_t *pps_register);


void PWM_SetFrequency(PWM_Timer timer, uint32_t freq_hz);


uint8_t PWM_GetDutyCycle(PWM_Channel channel);
# 9 "PWM.c" 2
# 18 "PWM.c"
static void calculate_timer_settings(uint32_t freq_hz, uint8_t *pr_value,
                                    PWM_Prescaler *prescaler) {
    uint32_t period;
    uint8_t prescale_values[] = {1, 4, 16, 64};


    for (int i = 0; i < 4; i++) {
        period = ((32000000 / 4) / (freq_hz * prescale_values[i])) - 1;

        if (period <= 255) {
            *pr_value = (uint8_t)period;
            *prescaler = (PWM_Prescaler)i;
            return;
        }
    }
...
void PWM_Init(PWM_Channel channel, PWM_Timer timer, uint32_t freq_hz) {
    ...
    calculate_timer_settings(freq_hz, &pr_value, &prescaler);


    switch(timer) {
        case PWM_TIMER2:
            if (T2CONbits.TMR2ON && PR2 == pr_value && T2CONbits.T2CKPS == prescaler) {
                timer_running = 1;
            }
            break;
        case PWM_TIMER4:
            if (T4CONbits.TMR4ON && PR4 == pr_value && T4CONbits.T4CKPS == prescaler) {
                timer_running = 1;
            }
            break;
        case PWM_TIMER6:
            if (T6CONbits.TMR6ON && PR6 == pr_value && T6CONbits.T6CKPS == prescaler) {
                timer_running = 1;
            }
            break;
    }
    ...
    switch(channel) {
        case PWM_CCP1:
            CCPTMRSbits.C1TSEL = timer;
            break;
        case PWM_CCP2:
            CCPTMRSbits.C2TSEL = timer;
            break;
    }
...
void PWM_SetDutyCycle(PWM_Channel channel, uint8_t duty_percent) {
    ...
    switch(channel) {
        case PWM_CCP1:
            switch(CCPTMRSbits.C1TSEL) {
                case 0b00: pr_value = PR2; break;
                case 0b01: pr_value = PR4; break;
                case 0b10: pr_value = PR6; break;
                default: pr_value = PR2;
            }
            break;
        case PWM_CCP2:
            switch(CCPTMRSbits.C2TSEL) {
                case 0b00: pr_value = PR2; break;
                case 0b01: pr_value = PR4; break;
                case 0b10: pr_value = PR6; break;
                default: pr_value = PR2;
    ...
    duty_10bit = (uint16_t)(((uint32_t)duty_percent * (pr_value + 1) * 4) / 100);

    PWM_SetDutyCycle10bit(channel, duty_10bit);
}
...
void PWM_SetDutyCycle10bit(PWM_Channel channel, uint16_t duty_value) {
    ...
    msb = (uint8_t)(duty_value >> 2);
    lsb = (uint8_t)(duty_value & 0x03);
...
uint8_t PWM_GetDutyCycle(PWM_Channel channel) {
    ...
    duty_10bit = ((uint16_t)msb << 2) | lsb;


    return (uint8_t)(((uint32_t)duty_10bit * 100) / ((pr_value + 1) * 4));
}
