...
if (SPI_Exchange8bit(0xFF) == 0xFF) break;
...
SPI_Exchange8bit(token);
if (token != 0xFD) {
i = 512;
do
SPI_Exchange8bit(*buff++);
while (--i);
SPI_Exchange8bit(0xFF);
SPI_Exchange8bit(0xFF);
resp = SPI_Exchange8bit(0xFF);
if ((resp & 0x1F) != 0x05)
return 0;
}
...
SPI_Exchange8bit(0x40 | cmd);
SPI_Exchange8bit((BYTE)(arg >> 24));
SPI_Exchange8bit((BYTE)(arg >> 16));
SPI_Exchange8bit((BYTE)(arg >> 8));
SPI_Exchange8bit((BYTE)arg);
n = 0x01;
if (cmd == (0)) n = 0x95;
if (cmd == (8)) n = 0x87;
SPI_Exchange8bit(n);
...
if (cmd == (12)) SPI_Exchange8bit(0xFF);
n = 10;
do
res = SPI_Exchange8bit(0xFF);
while ((res & 0x80) && --n);
...
SPI_Initialize();
for (n = 10; n; n--) SPI_Exchange8bit(0xFF);

ty = 0;
if (send_cmd((0), 0) == 1) {
if (send_cmd((8), 0x1AA) == 1) {
for (n = 0; n < 4; n++) ocr[n] = SPI_Exchange8bit(0xFF);
...
for (n = 64 - 16; n; n--) SPI_Exchange8bit(0xFF);
...
case 13 :
if (send_cmd((58), 0) == 0) {
for (n = 4; n; n--) *ptr++ = SPI_Exchange8bit(0xFF);
res = RES_OK;
}
break;
