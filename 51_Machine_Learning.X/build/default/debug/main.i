...
# 1 "./ml.h" 1
# 28 "./ml.h"
static const float ml_support_vectors[27][4] = {
...
static const char* ml_class_names[3] = {
    "Setosa",
    "Versicolor",
    "Virginica"
};
...
unsigned char parse_features(char *buffer, float *features)
{
    ...
    while (count < 4) {
        ...
        if (strlen(token) > 0) {
            features[count] = (float)strtof(token,0);
            count++;
        }
