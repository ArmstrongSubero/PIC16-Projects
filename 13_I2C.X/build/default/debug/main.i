...
static uint8_t EEPROM_ControlByte(uint16_t memAddr, _Bool read)
{
    uint8_t block = (memAddr >> 8) & 0x07;
    uint8_t ctrl = (0b1010 << 4) | (block << 1);
...
I2C_Status_t EEPROM_AckPoll(uint16_t memAddr)
{
    for (uint16_t t = 0; t < 200; t++)
    {
        ...
        st = I2C_WriteByte(EEPROM_ControlByte(memAddr, 0));
        I2C_Stop();
...
I2C_Status_t EEPROM_ReadByte(uint16_t memAddr, uint8_t *data)
{
    ...
    st = I2C_WriteByte(EEPROM_ControlByte(memAddr, 1));
    if (st != I2C_OK) { I2C_Stop(); return st; }
