...
static unsigned char nmea_checksum_ok(const char *s)
{
    if (!s || s[0] != '$') return 0;
...
static unsigned char gps_read_line(char *buf, unsigned char maxlen)
{
    unsigned char n = 0;
    char c;
...
void main(void) {
    ...
    suart_puts("\r\n=== NEO-6M BRIDGE TEST ===\r\n");
    suart_puts("HW UART (RB3/RB2) <-> GPS @ 9600\r\n");
    suart_puts("SoftUART (RB1) -> PC terminal\r\n\r\n");

    while(1)
    {
        gps_read_line(line, sizeof(line));


        suart_puts(line);


        if (nmea_checksum_ok(line)) suart_puts("[CS OK]\r\n");
        else suart_puts("[CS BAD]\r\n");
    }
