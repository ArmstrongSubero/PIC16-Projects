...
# 102 "main.c"
    ...
    RESTARTLEVEL:
        ...
        while(1) {
            ...
            if ((PORTAbits.RA2 == 0)) StartGame(&Sprite[0]);

            if (INGAME) {
                if ((PORTAbits.RA0 == 0)) Sprite[0].DirectionV = 0;
                else if ((PORTAbits.RA1 == 0)) Sprite[0].DirectionV = 1;

                if ((PORTAbits.RA4 == 0)) Sprite[0].DirectionH = 1;
                else if ((PORTAbits.RA5 == 0)) Sprite[0].DirectionH = 0;
...
void testJoy(void) {
    ...
    while(1) {
        ...
        if ((PORTAbits.RA0 == 0)) {
            OLED_data_start(0);
            for(uint8_t i = 0; i < 10; i++) OLED_send(0xFF);
            OLED_end();
        }

        if ((PORTAbits.RA1 == 0)) {
            OLED_data_start(0);
            for(uint8_t i = 0; i < 118; i++) OLED_send(0x00);
            for(uint8_t i = 0; i < 10; i++) OLED_send(0xFF);
            OLED_end();
        }
        ...
        if ((PORTAbits.RA5 == 0)) {
            OLED_data_start(0);
            for(uint8_t i = 0; i < 54; i++) OLED_send(0x00);
            for(uint8_t i = 0; i < 20; i++) OLED_send(0xFF);
            OLED_end();
        }
...
uint8_t CollisionPac2Caracter(PERSONAGE *Sprite) {
    ...
    if (INGAME) {
        for (uint8_t t = 1; t <= 4; t++) {
            if (((Sprite[0].x + 6) < (Sprite[t].x)) || ((Sprite[0].x) > (Sprite[t].x + 6)) || (((Sprite[0].y * 8) + Sprite[0].Decalagey + 6) < ((Sprite[t].y * 8) + Sprite[t].Decalagey)) || (((Sprite[0].y * 8) + Sprite[0].Decalagey) > ((Sprite[t].y * 8) + Sprite[t].Decalagey + 6))) {
...
uint8_t RecupeBacktoCompV(uint8_t SpriteCheck, PERSONAGE *Sprite) {
    ...
    if (Sprite[SpriteCheck].DirectionV == 1) {
        Y1 = back[(Sprite[SpriteCheck].y * 128) + (Sprite[SpriteCheck].x + 6)];
        Y2 = back[((Sprite[SpriteCheck].y + 1) * 128) + (Sprite[SpriteCheck].x + 6)];
    } else if (Sprite[SpriteCheck].DirectionV == 0) {
        Y1 = back[(Sprite[SpriteCheck].y * 128) + (Sprite[SpriteCheck].x)];
        Y2 = back[((Sprite[SpriteCheck].y + 1) * 128) + (Sprite[SpriteCheck].x)];
    } else {
        Y1 = 0;
        Y2 = 0;
    }
    ...
    for (y = 0; y < 8; y++) {
        ...
        for (x = 0; x < 128; x++) {
            if (render0_picture1 == 0) {
                if (INGAME) {
                    OLED_send(background(x, y) | SpriteWrite(x, y, Sprite) | DotsWrite(x, y, Sprite) | LiveWrite(x, y) | FruitWrite(x, y));
                } else {
                    OLED_send(0xff - (background(x, y) | SpriteWrite(x, y, Sprite)));
                }
            } else if (render0_picture1 == 1) {
                OLED_send(back[x + (y * 128)]);
            }
        }
        OLED_end();
    }
