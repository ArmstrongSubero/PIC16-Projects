...
static uint8_t buttonPressed(uint8_t pin_now, uint8_t *pin_prev)
{
    uint8_t pressed = 0;


    if((*pin_prev == 1) && (pin_now == 0))
        pressed = 1;
...
void main(void)
{
    ...
    while(1)
    {
        ...
        if(buttonPressed(PORTAbits.RA0, &prev_start))
        {
        ...
        if(buttonPressed(PORTAbits.RA1, &prev_inc))
        {
            if(displayValue < 9999) displayValue++;
            else displayValue = 0;
        }

        if(buttonPressed(PORTAbits.RA2, &prev_dec))
        {
