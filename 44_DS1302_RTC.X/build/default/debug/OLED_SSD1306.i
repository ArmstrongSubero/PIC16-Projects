...
static uint8_t screen_buffer[(32 / 8)][128];
# 182 "OLED_SSD1306.c"
static uint8_t SSD1306_CtrlW(void) { return (uint8_t)((0x3C << 1) | 0); }
...
static I2C_Status_t OLED_WriteData(const uint8_t* data, uint16_t len)
{
    ...
    if (data == ((void*)0)) return I2C_TIMEOUT;
...
void OLED_SetOrientation(oled_orient_t o)
{
    (void)OLED_Command(0x2E);

    switch (o)
    {
        default:
        case OLED_ORIENT_0:
            (void)OLED_Command(0xA0 | 0x01);
            (void)OLED_Command(0xC8);
            break;

        case OLED_ORIENT_180:
            (void)OLED_Command(0xA0 | 0x00);
            (void)OLED_Command(0xC0);
            break;
...
void OLED_Init(void)
{
    ...
    (void)OLED_Command(0xD5);
    (void)OLED_Command(0x80);

    (void)OLED_Command(0xA8);
    ...
    (void)OLED_Command(0xD3);
    (void)OLED_Command(0x00);

    (void)OLED_Command(0x40 | 0x00);

    (void)OLED_Command(0x8D);
    (void)OLED_Command(0x14);

    (void)OLED_Command(0x20);
    (void)OLED_Command(0x02);

    (void)OLED_Command(0xA0 | 0x01);
    (void)OLED_Command(0xC8);

    (void)OLED_Command(0xDA);
    ...
    (void)OLED_Command(0x81);
    (void)OLED_Command(0x8F);

    (void)OLED_Command(0xD9);
    (void)OLED_Command(0xF1);

    (void)OLED_Command(0xDB);
    (void)OLED_Command(0x40);

    (void)OLED_Command(0xA4);
    (void)OLED_Command(0xA6);
    (void)OLED_Command(0xAF);
...
void OLED_Clear(void)
{
    for (uint8_t p = 0; p < (32 / 8); p++)
    {
        memset(screen_buffer[p], 0, 128);

        OLED_YX(p, 0);
        (void)OLED_WriteData(screen_buffer[p], 128);
    }
}

static void OLED_ClearFast(void)
{
    static const uint8_t zeros[128] = {0};


    (void)OLED_Command(0x00);
    (void)OLED_Command(0x10);

    for (uint8_t p = 0; p < (32 / 8); p++)
    {
        (void)OLED_Command(0xB0 | p);


        (void)OLED_WriteData(zeros, 128);
    }
}
...
void OLED_UpdateLine(uint8_t page)
{
    ...
    OLED_YX(page, 0);
    (void)OLED_WriteData(screen_buffer[page], 128);
}
...
void OLED_UpdatePartialLine(uint8_t page, uint8_t start_col, uint8_t num_cols)
{
    if (page >= (32 / 8) || start_col >= 128) return;
    if ((uint16_t)start_col + num_cols > 128)
        num_cols = (uint8_t)(128 - start_col);
...
void OLED_Printf_Line(uint8_t line, const char* fmt, ...)
{
    ...
    *ap = __va_start();
    ...
    while (*src && (dst - buf) < 31) {
        if (*src == '%') {
            ...
            switch (*src) {
                case 'd': {
                    int16_t val = (*(int *)__va_arg(*(int **)ap, (int)0));
                    if (val < 0) {
                        *dst++ = '-';
                        val = -val;
                    }
                ...
                case 'l': {
                    src++;
                    if (*src == 'u') {
                        uint32_t val = (*(uint32_t *)__va_arg(*(uint32_t **)ap, (uint32_t)0));
                        char temp[11];
                        uint8_t i = 0;
                        do {
                            temp[i++] = '0' + (val % 10);
                        ...
                        int32_t val = (*(int32_t *)__va_arg(*(int32_t **)ap, (int32_t)0));
                        if (val < 0) {
                            *dst++ = '-';
                            val = -val;
                        }
                ...
                case 'u': {
                    uint16_t val = (*(unsigned int *)__va_arg(*(unsigned int **)ap, (unsigned int)0));
                    char temp[6];
                    uint8_t i = 0;
                    do {
                        temp[i++] = '0' + (val % 10);
                ...
                case 's': {
                    const char *str = (*(const char* *)__va_arg(*(const char* **)ap, (const char*)0));
                    while (*str && (dst - buf) < 31) {
                        *dst++ = *str++;
                    }
                    break;
                }
                case 'c': {
                    *dst++ = (char)(*(int *)__va_arg(*(int **)ap, (int)0));
                    break;
                }
                case '%': {
                    *dst++ = '%';
    ...
    *dst = '\0';
    ...
    memset(screen_buffer[line], 0, 128);


    uint8_t col = 0;
    for (const char* p = buf; *p && (col + 8) <= 128; ++p)
    {
        char ch = *p;
        if (ch < 32 || ch > 127) ch = ' ';
...
void OLED_DrawBattery(uint8_t percentage, uint8_t x, uint8_t page)
{
    ...
    for (uint8_t i = 1; i <= fill_w && i <= 6; i++) {
        screen_buffer[page][x + i] = BatteryIcon[i] | 0x7E;
    }
}
