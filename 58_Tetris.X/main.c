 ...
 * Program Version: 1.0
 *                
 * Program Description: This Program demonstrates Tetris on the PIC16 
 ...
 *                       connected to RA0 and RA1, RA4 is the down button 
 *                       
 * Created on Friday 39th, January, 2026, 07:41 PM
 */

// ===================================================================================
// Project:   Tiny Tris - PIC16F1718/1719 Port
// Version:   v1.1
// ===================================================================================
//
...
// ===================================================================================
// Frame timing constant - adjust if game is too fast/slow
// ===================================================================================
#define FRAME_DELAY_MS  15   // ~60 FPS for input screen updates slower
...
void main(void) {
    ...
    while(1) {
        ...
        // Hold DOWN at startup to save high score
        if (JOY_down_pressed()) {
            JOY_DLY_ms(1000);
            if (JOY_down_pressed()) {
    ...
    MENU:
        {
            ...
            while(1) {
                ...
                // Render every 6th frame (keeps screen update rate reasonable)
                if (SKIP_FRAME >= 6) {
                    Tiny_Flip_TTRIS(82);
                    SKIP_FRAME = 0;
                } else {
                ...
                // CRITICAL: Frame timing delay
                // Without this, the PIC runs too fast and timing variables
                // count down instantly, causing jerky movement
                JOY_DLY_ms(FRAME_DELAY_MS);
            }
        }
    }
}
...
void Game_Play_TTRIS(void) {
    ...
    if (Level_TTRIS < 21) {
        Level_Speed_ADJ_TTRIS = (uint8_t)map(Level_TTRIS, 0, 20, 15, 5);
    }
...
uint8_t recupe_SCORES_TTRIS(uint8_t xPASS, uint8_t yPASS) {
    ...
    // Extract digits using local variables (not macros)
...
void INIT_ALL_VAR_TTRIS(void) {
    ...
    DROP_TRIG_TTRIS = 15;           // Changed from 1
    Level_Speed_ADJ_TTRIS = 15;     // Add this line
    xx_TTRIS = 0;
    yy_TTRIS = 0;
...
void CONTROLE_TTRIS(uint8_t *Rot_TTRIS) {
    ...
    // Left/Right movement - FASTER RESPONSE
    if (SPEED_x_trig_TTRIS == 0) {
        if (JOY_right_pressed()) {
            if (LONG_PRESS_X_TTRIS == 0) { SND_TTRIS(1); }
            if ((LONG_PRESS_X_TTRIS == 0) || (LONG_PRESS_X_TTRIS >= 8)) {  // Was 20, now 8
                DEPLACEMENT_XX_TTRIS = 1;
                SPEED_x_trig_TTRIS = 1;  // Was 2, now 1
            }
            if (LONG_PRESS_X_TTRIS < 8) { LONG_PRESS_X_TTRIS++; }  // Was 20, now 8
        }
    ...
    if (!JOY_right_pressed() && !JOY_left_pressed()) {
        LONG_PRESS_X_TTRIS = 0;
        PSEUDO_RND_TTRIS();
    }
    ...
    // Fast drop with DOWN button - FASTER
    if (JOY_down_pressed()) {
        PSEUDO_RND_TTRIS();
        if (DOWN_DESACTIVE_TTRIS == 0) {
            DEPLACEMENT_YY_TTRIS = 1;
            DROP_TRIG_TTRIS = 1;  // Was 2, now 1 - instant repeat
        }
    } else {
        DOWN_DESACTIVE_TTRIS = 0;
    }
