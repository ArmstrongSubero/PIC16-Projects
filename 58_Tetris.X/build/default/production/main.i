    ...
    MENU:
        {
            ...
            while(1) {
                ...
                if ((PORTAbits.RA2 == 0) && (Ripple_filter_TTRIS == 0)) {
                    PSEUDO_RND_TTRIS();
...
void Game_Play_TTRIS(void) {
    uint8_t LEVEL_TMP = (uint8_t)(Nb_of_line_F_TTRIS / 20);
    if (Level_TTRIS != LEVEL_TMP) {
        Level_TTRIS = LEVEL_TMP;
        SND_TTRIS(2);
    }
    if (Level_TTRIS < 21) {
        Level_Speed_ADJ_TTRIS = (uint8_t)(((long)(Level_TTRIS) - (long)(0)) * ((long)(5) - (long)(15)) / ((long)(20) - (long)(0)) + (long)(15));
    }

    if (Level_Speed_ADJ_TTRIS < 5) {
        Level_Speed_ADJ_TTRIS = 5;
    }
}
...
uint8_t GRID_STAT_TTRIS(int8_t X_SCAN, int8_t Y_SCAN) {
    ...
    uint8_t Y_VAR_SELECT = (uint8_t)(Y_SCAN >> 3);
    uint8_t Y_VAR_DECALAGE = RecupeDecalageY_TTRIS((uint8_t)Y_SCAN);
...
uint8_t blitzSprite_TTRIS(int8_t xPos, int8_t yPos, uint8_t xPASS, uint8_t yPASS, uint8_t FRAME, const uint8_t *SPRITES) {
    ...
    uint8_t RECUPELINEY = RecupeLineY_TTRIS((uint8_t)yPos);
    ...
    uint8_t SPRITEyDECALAGE = RecupeDecalageY_TTRIS((uint8_t)yPos);
...
uint8_t SplitSpriteDecalageY_TTRIS(uint8_t decalage, uint8_t Input, uint8_t UPorDOWN) {
    if (UPorDOWN) {
        return (uint8_t)(Input << decalage);
    }
    return (uint8_t)(Input >> (8 - decalage));
}
...
uint8_t RecupeDecalageY_TTRIS(uint8_t Valeur) {
    return (uint8_t)(Valeur - ((Valeur >> 3) << 3));
}
...
void Flip_intro_TTRIS(uint8_t *TIMER1) {
    ...
    for (y = 0; y < 8; y++) {
        OLED_data_start(y);
        for (x = 0; x < 128; x++) {
            OLED_send(intro_TTRIS(x, y, TIMER1));
        }
        OLED_end();
    }
}
...
uint8_t recupe_SCORES_TTRIS(uint8_t xPASS, uint8_t yPASS) {
    ...
    uint16_t temp = Scores_TTRIS;
    uint8_t d10000 = (uint8_t)(temp / 10000);
    temp -= (uint16_t)d10000 * 10000;
    uint8_t d1000 = (uint8_t)(temp / 1000);
    temp -= (uint16_t)d1000 * 1000;
    uint8_t d100 = (uint8_t)(temp / 100);
    temp -= (uint16_t)d100 * 100;
    uint8_t d10 = (uint8_t)(temp / 10);
    uint8_t d1 = (uint8_t)(temp - (uint16_t)d10 * 10);

    return (blitzSprite_TTRIS(95, 8, xPASS, yPASS, d10000, police_TTRIS) |
            blitzSprite_TTRIS(99, 8, xPASS, yPASS, d1000, police_TTRIS) |
            blitzSprite_TTRIS(103, 8, xPASS, yPASS, d100, police_TTRIS) |
            blitzSprite_TTRIS(107, 8, xPASS, yPASS, d10, police_TTRIS) |
            blitzSprite_TTRIS(111, 8, xPASS, yPASS, d1, police_TTRIS) |
            blitzSprite_TTRIS(115, 8, xPASS, yPASS, 0, police_TTRIS));
}

void Convert_Nb_of_line_TTRIS(void) {
    Nb_of_line_TTRIS[2] = (uint8_t)(Nb_of_line_F_TTRIS / 100);
    Nb_of_line_TTRIS[1] = (uint8_t)((Nb_of_line_F_TTRIS - (Nb_of_line_TTRIS[2] * 100)) / 10);
    Nb_of_line_TTRIS[0] = (uint8_t)(Nb_of_line_F_TTRIS - (Nb_of_line_TTRIS[2] * 100) - (Nb_of_line_TTRIS[1] * 10));
}
...
void CONTROLE_TTRIS(uint8_t *Rot_TTRIS) {
    ...
    if (SPEED_x_trig_TTRIS == 0) {
        if ((PORTAbits.RA1 == 0)) {
            ...
            if ((LONG_PRESS_X_TTRIS == 0) || (LONG_PRESS_X_TTRIS >= 8)) {
                DEPLACEMENT_XX_TTRIS = 1;
                SPEED_x_trig_TTRIS = 1;
            }
            if (LONG_PRESS_X_TTRIS < 8) { LONG_PRESS_X_TTRIS++; }
        }
...
void Move_Piece_TTRIS(void) {
    ...
    if (DEPLACEMENT_XX_TTRIS != 0) {
        if (Check_collision_x_TTRIS(DEPLACEMENT_XX_TTRIS) == 0) {
            xx_TTRIS += DEPLACEMENT_XX_TTRIS;
        }
        DEPLACEMENT_XX_TTRIS = 0;
    }


    if (DEPLACEMENT_YY_TTRIS != 0) {
        Ou_suis_Je_TTRIS(xx_TTRIS, yy_TTRIS);
        if (Check_collision_y_TTRIS(1)) {
            DEPLACEMENT_YY_TTRIS = 0;
            LONG_PRESS_X_TTRIS = 0;
            Ripple_filter_TTRIS = 0;
            DROP_BREAK_TTRIS = 6;
