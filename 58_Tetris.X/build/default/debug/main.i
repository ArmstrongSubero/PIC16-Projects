...
if (DROP_BREAK_TTRIS==6) {
  END_DROP_TTRIS();
  if (End_Play_TTRIS()) { Tiny_Flip_TTRIS(128);SND_TTRIS(3); _delay((unsigned long)((2000)*(32000000/4000.0)));Check_NEW_RECORD();goto MENU;}
...
if (((PORTAbits.RA2 == 0))&&(Ripple_filter_TTRIS==0)) {PSEUDO_RND_TTRIS();Ripple_filter_TTRIS=1;}
...
if ((PORTAbits.RA2 == 0)) {reset_Score_TTRIS();break;}
...
if (((PORTAbits.RA1 == 0)==0)&&((PORTAbits.RA0 == 0)==0)) {LONG_PRESS_X_TTRIS=0;PSEUDO_RND_TTRIS();}
...
if (Level_TTRIS<21) {Level_Speed_ADJ_TTRIS=(((long)(Level_TTRIS) - (long)(0)) * ((long)(1) - (long)(11)) / ((long)(20) - (long)(0)) + (long)(11));}
}
...
for (y = 0; y < 8; y++){
OLED_data_start(y);
for (x = 0; x < HR_TTRIS; x++){OLED_send(Recupe_TTRIS(x,y));}
OLED_end();
}}
...
for (y = 0; y < 8; y++){
OLED_data_start(y);
for (x = 0; x < 128; x++){OLED_send(intro_TTRIS(x,y,TIMER1));}
OLED_end();
}}
...
uint8_t d10000 = temp / 10000;
temp -= (uint16_t)d10000 * 10000;
uint8_t d1000 = temp / 1000;
temp -= (uint16_t)d1000 * 1000;
uint8_t d100 = temp / 100;
temp -= (uint16_t)d100 * 100;
uint8_t d10 = temp / 10;
uint8_t d1 = temp - (uint16_t)d10 * 10;

return
(blitzSprite_TTRIS(95,8,xPASS,yPASS,d10000,police_TTRIS)|
