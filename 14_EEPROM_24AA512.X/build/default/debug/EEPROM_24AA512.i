...
# 2 "<built-in>" 2
# 1 "EEPROM_24AA512.c" 2
# 13 "EEPROM_24AA512.c"
# 1 "./EEPROM_24AA512.h" 1
# 12 "./EEPROM_24AA512.h"
...
# 13 "EEPROM_24AA512.c" 2





static uint8_t EEPROM_Ctrl(_Bool read)
{
    return (uint8_t)((0x50 << 1) | (read ? 1 : 0));
}
...
static uint8_t EEPROM24AA512_ControlByte(_Bool read)
{
    uint8_t ctrl = (0b1010 << 4) | ((0 & 0x07) << 1);
    return (uint8_t)(ctrl | (read ? 1 : 0));
}
# 42 "EEPROM_24AA512.c"
I2C_Status_t EEPROM24AA512_AckPoll(void)
{
    ...
    {
        I2C_Start();
        I2C_Status_t st = I2C_WriteByte(EEPROM_Ctrl(0));
        I2C_Stop();
...
I2C_Status_t EEPROM24AA512_WriteByte(uint16_t addr, uint8_t data)
{
    ...
    st = I2C_WriteByte(EEPROM_Ctrl(0));
    if (st != I2C_OK) { I2C_Stop(); return st; }

    st = I2C_WriteByte((uint8_t)(addr >> 8));
    if (st != I2C_OK) { I2C_Stop(); return st; }

    st = I2C_WriteByte((uint8_t)(addr & 0xFF));
    if (st != I2C_OK) { I2C_Stop(); return st; }
...
I2C_Status_t EEPROM24AA512_ReadByte(uint16_t addr, uint8_t *data)
{
    ...
    st = I2C_WriteByte(EEPROM_Ctrl(1));
    if (st != I2C_OK) { I2C_Stop(); return st; }

    st = I2C_ReadByte(data, 0);
    I2C_Stop();
...
I2C_Status_t EEPROM24AA512_WritePage(uint16_t addr, const uint8_t *data, uint8_t len)
{
    ...
    uint8_t offset = (uint8_t)(addr & 0x7F);
    uint8_t room = (uint8_t)(128 - offset);
    ...
    st = I2C_WriteByte(EEPROM24AA512_ControlByte(0));
    if (st != I2C_OK) { I2C_Stop(); return st; }
...
I2C_Status_t EEPROM24AA512_ReadSequential(uint16_t addr, uint8_t *data, uint16_t len)
{
    ...
    st = I2C_WriteByte(EEPROM24AA512_ControlByte(1));
    if (st != I2C_OK) { I2C_Stop(); return st; }

    for (uint16_t i = 0; i < len; i++)
    {
        _Bool ack = (i < (len - 1));
        st = I2C_ReadByte(&data[i], ack);
        if (st != I2C_OK) { I2C_Stop(); return st; }
    }
