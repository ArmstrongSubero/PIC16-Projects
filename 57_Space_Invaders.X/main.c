 ...
 * Program: 57_Space_Invaders
 * Compiler: XC8 (v2.46, MPLAX X v6.20)
 * Program Version: 1.0
 *                
 * Program Description: This Program demonstrates space invaders on the PIC16 
 *                      specifically the "Tiny Invaders" port optimized for 
 ...
 *                       connected to RA0 and RA1. 
 *                       
 * Created on Friday 30th, January, 2026, 05:42 PM
 *
 * Original: T-I-N-Y  I-N-V-A-D-E-R-S v3.1 for ATTINY85  GPLv3
 ...
 * 
 * Target: PIC16F1718 / PIC16F1719 @ 32MHz
 * Compiler: XC8
 */
    ...
    Bypass:
        ...
        while(1) {
            ...
            if(ShipDead != 1) {
                ...
                if(JOY_left_pressed()) { if(VarPot > 5) VarPot = VarPot - 6; }
                if(JOY_right_pressed()) { if(VarPot < 108) VarPot = VarPot + 6; }
...
uint8_t LivePrint(uint8_t x, uint8_t y) {
    ...
    if((0 >= (x - XLIVEWIDE)) && (y == 7)) return LIVE[x];
    return 0x00;
}
    ...
    for(y = 0; y < 8; y++) {
        ...
        for(x = 0; x < 128; x++) {
            if(render0_picture1 == 0) {
                ...
                JOY_OLED_send(background(x, y, space) | LivePrint(x, y) | Vesso(x, y, space)
...
void Monster_Attack_Check(SPACE *space) {
    ...
    #define Ymouin   (space->MonsterGroupeYpos << 3)
