...
static uint8_t pn532_packetbuffer[255 + 16];
...
static void I2C_BusRecover_RC4_RC5(void)
{
    ...
    for (uint8_t i = 0; i < 9; i++) {
        LATCbits.LATC5 = 0; _delay((unsigned long)((10)*(32000000/4000000.0)));
        LATCbits.LATC5 = 1; _delay((unsigned long)((10)*(32000000/4000000.0)));
    }

    TRISCbits.TRISC4 = 0;
    LATCbits.LATC4 = 0; _delay((unsigned long)((10)*(32000000/4000000.0)));
    LATCbits.LATC5 = 1; _delay((unsigned long)((10)*(32000000/4000000.0)));
    LATCbits.LATC4 = 1; _delay((unsigned long)((10)*(32000000/4000000.0)));
...
static I2C_Status_t pn532_i2c_write_raw(const uint8_t *data, uint16_t len)
{
    ...
    st = I2C_WriteByte((uint8_t)((0x24 << 1) | 0u));
...
static I2C_Status_t pn532_i2c_read_raw(uint8_t *out, uint16_t len)
{
    ...
    st = I2C_WriteByte((uint8_t)((0x24 << 1) | 1u));
    if (st != I2C_OK) { (void)I2C_Stop(); return st; }

    st = I2C_ReadByte(&status, 1);
    if (st != I2C_OK) { (void)I2C_Stop(); return st; }
...
static _Bool pn532_wait_ready(uint32_t timeout_ms)
{
    while (timeout_ms--)
    {
        uint8_t status = 0;
        ...
        st = I2C_ReadByte(&status, 0);
        (void)I2C_Stop();

        if (st == I2C_OK && status == 0x01)
            return 1;
                              ...
                              uint8_t *resp, uint8_t resp_len, uint16_t timeout_ms)
{
    ...
    if (PN532_WriteFrame(frame_data, frame_len) != (0)) {
        return (-1);
    }
...
int PN532_GetFirmwareVersion(uint8_t *version)
{
    ...
    int result = PN532_CallFunction(0x02,
                                    ((void*)0), 0,
                                    version, 4,
                                    500);
...
int PN532_SamConfiguration(void)
{
    ...
    int result = PN532_CallFunction(0x14,
...
int PN532_ReadPassiveTarget(uint8_t *uid, uint8_t card_baud, uint32_t timeout_ms)
{
    ...
    int result = PN532_CallFunction(0x4A,
                                    params, sizeof(params),
                                    resp, sizeof(resp),
                                    (uint16_t)timeout_ms);
...
int PN532_Ntag2xxReadBlock(uint8_t* response, uint8_t block_number)
{
    ...
    uint8_t params[3] = {0x01, 0x30, block_number};
    uint8_t buff[16 + 1];

    int result = PN532_CallFunction(0x40,
                                    params, sizeof(params),
                                    buff, sizeof(buff),
                                    1000);
...
int PN532_Ntag2xxFastRead(uint8_t* response, uint8_t start_page, uint8_t end_page)
{
    ...
    uint8_t params[4] = {0x01, 0x3A, start_page, end_page};
    uint8_t buff[64];
...
static uint8_t mf_buffer[16];
static uint8_t mf_current_uid[10];
static uint8_t mf_current_uid_len;
static uint8_t mf_ndef_data[64];
static char mf_text[32];
                                    ...
                                    char* text_out, uint8_t max_len)
{
    ...
    for (k = 0; k < 5; k++) {
        ...
        sel = PN532_ReadPassiveTarget(mf_current_uid, 0x00, 500);
        if (sel <= 0) continue;
...
void PN532_DumpMifareClassic(const uint8_t* uid, uint8_t uid_len)
{
    ...
    for (uint8_t sector = 0; sector < 16; sector++) {
        ...
        for (uint8_t k = 0; k < 5 && !authenticated; k++) {

            sel_result = PN532_ReadPassiveTarget(mf_current_uid, 0x00, 500);
            if (sel_result <= 0) {
                         ...
                         char *out, uint8_t out_size)
{
    ...
    uint8_t copy_len = ((text_len)<(out_size - 1)?(text_len):(out_size - 1));
...
void PN532_DumpNtagCard(void)
{
    ...
    if (PN532_GetNdefText(text, sizeof(text)) == (0)) {
        EUSART_Write_Text("\nNDEF Text: ");
        EUSART_Write_Text(text);
        EUSART_Write_Text("\n");
    } else {
