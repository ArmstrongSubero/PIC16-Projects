...
# 2 "main.c" 2
# 13 "main.c"
static const uint8_t digit_mask[4] = { 0b00000010u, 0b00000100u, 0b00001000u, 0b00010000u };
...
volatile uint32_t total_cs = 1000;
volatile uint8_t tick10ms = 0;




static __attribute__((inline)) uint8_t seg(uint8_t num)
{
    switch(num)
    {
        case 0: return 0x3F;
...
{
    uint32_t seconds = cs / 100;
    uint32_t hund = cs % 100;
...
static void btn_init(hold_btn_t *b)
{
    b->stable = 1;
    b->last_raw = 1;
    b->db_count = 0;
...
static uint8_t btn_debounce_10ms(hold_btn_t *b, uint8_t raw_now)
{
    if(raw_now == b->last_raw) {
        if(b->db_count < 3) b->db_count++;
    } else {
        b->db_count = 0;
        b->last_raw = raw_now;
    }
...
{
    if(hold_ms < 600) return 200;
    if(hold_ms < 1200) return 120;
    if(hold_ms < 2500) return 70;
    return 40;
}


static uint8_t btn_action_10ms(hold_btn_t *b, uint8_t stable_now)
{
    static uint8_t prev = 1;
    uint8_t fire = 0;


    if(prev == 1 && stable_now == 0) {
        b->hold_ms = 0;
        b->rep_ms = 0;
        fire = 1;
    }
    ...
    if(stable_now == 0) {
        ...
        if(b->hold_ms >= 350) {
            b->rep_ms += 10;
            uint16_t iv = accel_interval_ms(b->hold_ms);
            if(b->rep_ms >= iv) {
                b->rep_ms = 0;
                fire = 1;
            }
...
void initMain(void)
{
    ...
    (INTCONbits.GIE = 0); updateDigits_SS_tt(total_cs); (INTCONbits.GIE = 1);
...
void main(void)
{
    ...
    while(1)
    {
        ...
        if((prev_start == 1) && (PORTAbits.RA0 == 0))
        {
        ...
        if(!running)
        {
            uint8_t inc_stable = btn_debounce_10ms(&inc_btn, PORTAbits.RA1);
            uint8_t dec_stable = btn_debounce_10ms(&dec_btn, PORTAbits.RA2);

            if(btn_action_10ms(&inc_btn, inc_stable))
            {
                if(total_cs <= 9899) total_cs += 100; else total_cs = 9999;
            }

            if(btn_action_10ms(&dec_btn, dec_stable))
            {
                if(total_cs >= 100) total_cs -= 100; else total_cs = 0;
            }
        }
