...
volatile uint32_t total_cs = 2000;
...
{
    if(hold_ms < 800) return 200;
    if(hold_ms < 1600) return 120;
    if(hold_ms < 3000) return 70;
    return 40;
}
...
void main(void)
{
    ...
    while(1)
    {
        ...
        if(pressed_edge_active_low(PORTAbits.RA0, &prev_start))
        {
            running = !running;
        ...
        if(!running)
        {
            if(hold_button_action(PORTAbits.RA1, &incBtn, 10))
            {
            ...
            if(hold_button_action(PORTAbits.RA2, &decBtn, 10))
            {
