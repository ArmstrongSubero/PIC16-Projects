...
void initMain() {
    ...
    Lcd_Clear();
    Lcd_Set_Cursor(1, 1);
    Lcd_Write_String("Inductance Meter");
...
void calibrate() {
    ...
    float frequency = ((float)total_counts * 16) / 1.0;

    if (frequency > 0) {
        float capacitance = 100e-9;
        float precomputed_constant = 4 * 3.14159 * 3.14159 * capacitance;
        float inductance_h = 1.0 / (precomputed_constant * frequency * frequency);
        calibration_value = (uint8_t)(inductance_h * 1e6);

        Lcd_Clear();
        Lcd_Set_Cursor(1, 1);
        Lcd_Write_String("Cal Done");
        Lcd_Set_Cursor(2, 1);
        Lcd_Write_String("Offset:");
        Lcd_Write_Integer(calibration_value);
        Lcd_Write_String("uH");
        _delay((unsigned long)((2000)*(32000000/4000.0)));
    } else {
        Lcd_Clear();
        Lcd_Write_String("Cal Error");
        _delay((unsigned long)((2000)*(32000000/4000.0)));
    }
}

void displayResults(float frequency, float inductance_uh) {
    Lcd_Clear();
    Lcd_Set_Cursor(1, 1);


    Lcd_Write_String("Freq: ");
    Lcd_Write_Integer((uint32_t)frequency);
    Lcd_Write_String(" Hz");


    Lcd_Set_Cursor(2, 1);
    if (inductance_uh > 999.0) {
        float inductance_mh = inductance_uh / 1000.0;
        Lcd_Write_String("L: ");
...
void main(void) {
    ...
    float frequency, inductance_h, inductance_uh;
    ...
    while (1) {
        ...
        frequency = ((float)total_counts * 16) / 1.0;

        if (frequency > 0) {
            inductance_h = 1.0 / (precomputed_constant * frequency * frequency);
            inductance_uh = (inductance_h * 1e6) - calibration_value;

            displayResults(frequency, inductance_uh);
        } else {
            Lcd_Clear();
            Lcd_Write_String("Low Frequency");
            _delay((unsigned long)((2000)*(32000000/4000.0)));
        }
    }
}
